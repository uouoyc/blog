...
export async function searchPosts(keyword: string): Promise<SearchResult[]> {
  ...
  const results = data.filter(item => {
    ...
    const descriptionMatch = item.meta.description?.toLowerCase().includes(searchTerm) || false;
  ...
  return results.map(item => {
    ...
    return {
      ...
      meta: {
        ...item.meta,
        title: titleMatch ? highlightText(item.meta.title, searchTerm, 100) : item.meta.title
      },
...
// 生成高亮摘录
function generateHighlightedExcerpt(item: SearchResult, searchTerm: string): string {
  const maxLength = 150;

  if (item.meta.description && item.meta.description.toLowerCase().includes(searchTerm)) {
...
// 高亮文本中的关键词
function highlightText(text: string, searchTerm: string, maxLength: number): string {
  let result = text;
